-- Fix: manual_projects sync in create_resume_full and save_resume_full
-- Root cause: CASE expression inside jsonb_array_elements with WITH ORDINALITY
-- causes PostgreSQL to infer `p` as record type instead of jsonb, breaking ->> operator.
-- Fix: extract projects array into a local variable before passing to jsonb_array_elements.

BEGIN;

-- =====================================================
-- 1. Replace create_resume_full
-- =====================================================
...
BEGIN
  ...
  IF v_resume_id IS NOT NULL THEN
    ...
    SELECT v_resume_id,
      NULLIF(NULLIF(btrim(elem->>'id'), ''), ''),
      NULLIF(btrim(elem->>'name'), ''),
      NULLIF(btrim(elem->>'role'), ''),
      NULLIF(btrim(elem->>'description'), ''),
      NULLIF(btrim(elem->>'techStack'), ''),
      NULLIF(btrim(elem->>'link'), ''),
      NULLIF(btrim(elem->>'period'), ''),
      (o - 1)::integer
